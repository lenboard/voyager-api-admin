...
class TranslationsTableSeeder extends Seeder
{
    ...
    private function categoriesTranslations()
    {
        ...
        //
        $cat = Category::where('slug', 'knowledge')->firstOrFail();
        if ($cat->exists) {
            $this->trans('pt', $this->arr(['categories', 'slug'], $cat->id), 'knowledge');
            $this->trans('pt', $this->arr(['categories', 'name'], $cat->id), 'Knowledge');
        }
        $cat = Category::where('slug', 'news')->firstOrFail();
        if ($cat->exists) {
            $this->trans('pt', $this->arr(['categories', 'slug'], $cat->id), 'news');
            $this->trans('pt', $this->arr(['categories', 'name'], $cat->id), 'News');
        }
        $cat = Category::where('slug', 'video')->firstOrFail();
        if ($cat->exists) {
            $this->trans('pt', $this->arr(['categories', 'slug'], $cat->id), 'video');
            $this->trans('pt', $this->arr(['categories', 'name'], $cat->id), 'Video');
        }
    }
